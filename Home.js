...
window.addEventListener('wheel', (event) => {
  ...
  scrollCooldown = true;
  setTimeout(() => (scrollCooldown = false), scrollDelay);

if (event.deltaY > 30) {
  currentIndex = (currentIndex + 1) % images.length; // วนไปข้างหน้า
  showImage(currentIndex);
} else if (event.deltaY < -30) {
  currentIndex = (currentIndex - 1 + images.length) % images.length; // วนย้อนกลับ
  showImage(currentIndex);
}
...
// ✅ เปลี่ยนรูปเมื่อคลิกหรือแตะหน้าจอ
...
document.addEventListener("DOMContentLoaded", () => {
  // ตอนโหลดหน้าให้มี warp-in effect
  ...
  links.forEach(link => {
    link.addEventListener("click", e => {
      ...
      // ใส่ warp-out
      document.body.classList.remove("warp-in");
      document.body.classList.add("warp-out");

      // รอ 600ms แล้วเปลี่ยนหน้า
      setTimeout(() => {
